...
import { genId, maskSecret, encryptSecret, clientIp, ipInCidr } from "../../common/utils/index.js";
...
export class AdminService {
  ...
  async revokeSession(user: any, sessionId: string) { this.requireAdmin(user); await this.prisma.session.update({ where: { id: sessionId }, data: { revoked: true, revokedReason: "admin_revoked" } }); await this.events.record({ type: "admin.session.revoked", actor: user, message: "吊销登录会话", metadata: { sessionId }, timeline: false }); return { ok: true }; }
  ...
  async assignProject(user: any, userId: string, body: any) { this.requireAdmin(user); const m = await this.prisma.projectMember.create({ data: { id: genId("pm"), projectId: body.projectId, userId, role: body.role || "editor" } }); await this.events.record({ type: "admin.project.assigned", actor: user, projectId: body.projectId, message: "分配用户到项目", metadata: { userId, role: m.role } }); return { member: m }; }
  async removeProject(user: any, userId: string, projectId: string) { this.requireAdmin(user); await this.prisma.projectMember.deleteMany({ where: { userId, projectId } }); await this.events.record({ type: "admin.project.unassigned", actor: user, projectId, message: "移除用户项目权限", metadata: { userId } }); return { ok: true }; }
  ...
  async updateRolePermissions(user: any, roleId: string, body: any) { this.requireAdmin(user); const role = await this.prisma.roleTemplate.update({ where: { id: roleId }, data: { permissions: body.permissions, ...(body.name ? { name: body.name } : {}) } }); await this.events.record({ type: "admin.role.updated", actor: user, message: `更新角色权限：${role.name}`, metadata: { roleId, permissionCount: role.permissions.length }, timeline: false }); return { role }; }
  ...
  async createKey(user: any, body: any) { this.requireAdmin(user); const secret = String(body.secret || ""); if (!secret) throw new ForbiddenException("通知 Key 不能为空"); const key = await this.prisma.notificationKey.create({ data: { id: genId("nk"), name: body.name || "未命名密钥", channelId: body.channelId || "", type: body.type || "webhook", secretEncrypted: encryptSecret(secret), secretMasked: maskSecret(secret), enabled: body.enabled !== false } }); return { key: { id: key.id, name: key.name, channelId: key.channelId, type: key.type, enabled: key.enabled, secretMasked: key.secretMasked } }; }
  ...
  async testNotification(user: any, body: any) { this.requireAdmin(user); return { ok: true, message: "测试发送已排队" }; }
  ...
  async createAcceptanceItem(user: any, projectId: string, body: any) { this.requireAdmin(user); const item = await this.prisma.acceptanceItem.create({ data: { id: genId("ai"), projectId, title: body.title, status: "PENDING", note: body.note || "" } }); return { item }; }
  async updateAcceptanceItem(user: any, itemId: string, body: any) { this.requireAdmin(user); const item = await this.prisma.acceptanceItem.update({ where: { id: itemId }, data: body }); return { item }; }
  async deleteAcceptanceItem(user: any, itemId: string) { this.requireAdmin(user); await this.prisma.acceptanceItem.delete({ where: { id: itemId } }); return { ok: true }; }
