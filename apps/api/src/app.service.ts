...
export class AppService {
  ...
  listProjects(user: User, filter?: string, group?: string) {
    ...
    if (group) projects = projects.filter(p => p.group === group);
    else if (filter === "mine") projects = projects.filter(p => p.ownerId === user.id);
    ...
    else projects = projects.filter(p => p.status === "ACTIVE");
  ...
  // --- Project groups ---
  projectGroups(user: User) { const projects = this.visibleProjects(user); const groups = [...new Set(projects.map(p => p.group))]; return { groups: groups.map(g => ({ name: g, count: projects.filter(p => p.group === g).length })) }; }

  // --- Dashboard with action links ---
  dashboardFull(user: User) { const projects = this.visibleProjects(user); const projectIds = new Set(projects.map(p => p.id)); const progress = this.progress.filter(p => projectIds.has(p.projectId)); const myProgress = progress.filter(p => p.userId === user.id); const pendingTasks = this.tasks.filter(t => projectIds.has(t.projectId) && (t.status === "TODO" || t.status === "BLOCKED")); return { metrics: { activeProjects: projects.filter(p => p.status === "ACTIVE").length, todayActions: pendingTasks.length, riskProjects: projects.filter(p => p.risk !== "low").length, pendingFiles: this.files.filter(f => projectIds.has(f.projectId) && !f.deleted).length, myCompletion: myProgress.length ? Math.round(myProgress.filter(p => p.status === "COMPLETED").length / myProgress.length * 100) : 0, myDeltaDays: myProgress.reduce((sum, p) => sum + (p.deltaDays || 0), 0) }, pendingActions: pendingTasks.slice(0, 10).map(t => ({ taskId: t.id, title: t.title, status: t.status, projectId: t.projectId, projectName: projects.find(p => p.id === t.projectId)?.name || "", action: t.status === "BLOCKED" ? "解除阻塞" : "开始处理" })), riskItems: projects.filter(p => p.risk !== "low").slice(0, 5).map(p => ({ projectId: p.id, name: p.name, risk: p.risk, progress: p.progress })), myProgress: myProgress.slice(0, 20) }; }

  // --- Task with full detail ---
  getTaskFull(user: User, taskId: string) { const task = this.tasks.find(t => t.id === taskId && t.status !== "DELETED"); if (!task || !this.canAccess(user, task.projectId)) throw new NotFoundException("任务不存在或无权限"); return { task: { ...task, progressItems: this.progress.filter(p => p.taskId === task.id).map(p => ({ ...p, user: this.publicUser(this.users.find(u => u.id === p.userId)!) })), submissions: this.submissions.filter(s => (s as any).taskId === task.id && !s.deleted), timeline: this.timeline.filter(t => t.projectId === task.projectId).slice(0, 20) } }; }

  // --- File collection by member ---
  fileCollection(user: User, projectId: string) { this.requireProject(user, projectId); const submissions = this.submissions.filter(s => (s as any).projectId === projectId && !s.deleted); const byMember: Record<string, any[]> = {}; submissions.forEach(s => { if (!byMember[s.userId]) byMember[s.userId] = []; byMember[s.userId].push(s); }); return { collection: Object.entries(byMember).map(([userId, items]) => ({ userId, userName: this.users.find(u => u.id === userId)?.name || "未知", count: items.length, submitted: items.filter((i: any) => i.status === "SUBMITTED" || i.status === "ACCEPTED").length, items })), total: submissions.length }; }
}
