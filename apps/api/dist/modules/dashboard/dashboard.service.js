...
let DashboardService = class DashboardService {
    ...
    async canEditProject(user, projectId) {
        if (user.role === "SUPER_ADMIN")
            return true;
    ...
    async dashboardStats(user) {
        ...
        const [projects, tasks, progressItems] = await Promise.all([
            this.prisma.project.findMany({
                where: { id: { in: visibleIds }, status: { not: "DELETED" } },
                orderBy: [{ createdAt: "asc" }, { id: "asc" }],
            }),
            this.prisma.task.findMany({
                where: { projectId: { in: visibleIds }, status: { notIn: ["DELETED", "ARCHIVED"] } },
                orderBy: [{ projectId: "asc" }, { sortOrder: "asc" }, { createdAt: "asc" }, { id: "asc" }],
            }),
            this.prisma.taskProgress.findMany({
                where: { projectId: { in: visibleIds }, status: { not: "DELETED" } },
        ...
        for (const item of progressItems) {
            if (item.status !== "BLOCKED" && item.status !== "DELAYED")
                continue;
    ...
    async dashboardFull(user) {
        ...
        const tasks = await this.prisma.task.findMany({
            where: { projectId: { in: [...projectIds] }, status: { notIn: ["DELETED", "ARCHIVED"] } },
            orderBy: [{ projectId: "asc" }, { sortOrder: "asc" }, { createdAt: "asc" }, { id: "asc" }],
        });
    ...
    async ganttV2(user) {
        ...
        const ensureDuration = (s, e) => {
            if (!s)
                return { start: todayStr(), end: todayStr() };
            if (!e || e === s) {
                const d = new Date((e || s) + "T00:00:00Z");
        ...
        const data = projects.map(project => {
            ...
            const children = projectTasks.map(t => {
                ...
                (t.dependencyIds || []).forEach((depId) => {
                    if (tasks.find(dt => dt.id === depId))
                        links.push({ id: `ln_${depId}_${t.id}`, from: depId, to: t.id, type: "FS" });
                });
    ...
    async syncGantt(user, body) {
        ...
        for (const move of moves) {
            ...
            if (!(await this.canEditProject(user, task.projectId)))
                throw new ForbiddenException("无甘特图编辑权限");
        ...
        for (const entry of links) {
            ...
            if (sourceTask.projectId !== targetTask.projectId)
                throw new ForbiddenException("暂不支持跨项目依赖");
            if (!(await this.canEditProject(user, targetTask.projectId)))
                throw new ForbiddenException("无甘特图编辑权限");
