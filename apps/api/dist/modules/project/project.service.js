...
let ProjectService = class ProjectService {
    ...
    visibleProjectsWhere(user) {
        const base = { status: { not: "DELETED" } };
        if (user.role !== "SUPER_ADMIN")
            base.members = { some: { userId: user.id } };
        return base;
    }
    async memberOf(user, projectId) {
        if (user.role === "SUPER_ADMIN")
            return true;
    ...
    async canManage(user, projectId) {
        if (user.role === "SUPER_ADMIN")
            return true;
    ...
    // ---- Projects ----
    async listProjects(user, filter, group) {
        const where = this.visibleProjectsWhere(user);
        if (group)
            where.group = group;
        else if (filter === "mine")
            ...
            where.acceptanceStatus = { in: ["pending", "in_review"] };
        ...
        const projects = await this.prisma.project.findMany({
            where: where,
            include: { members: true, tasks: { where: { status: { notIn: ["DELETED", "ARCHIVED"] } } } },
    ...
    async projectDetail(user, id) {
        ...
        const [members, tasks, files, timeline] = await Promise.all([
            ...
            this.prisma.task.findMany({
                where: { projectId: id, status: { notIn: ["DELETED", "ARCHIVED"] } },
    ...
    async updateProject(user, id, body) {
        await this.requireProject(user, id);
        if (!(await this.canManage(user, id)))
            throw new ForbiddenException("无项目管理权限");
    ...
    async archiveProject(user, id) {
        await this.requireProject(user, id);
        if (!(await this.canManage(user, id)))
            throw new ForbiddenException("无项目归档权限");
    ...
    async restoreProject(user, id) {
        ...
        if (!project)
            throw new NotFoundException("项目不存在");
        if (!(await this.canManage(user, id)))
            throw new ForbiddenException("无项目恢复权限");
    ...
    async deleteProject(user, id) {
        await this.requireProject(user, id);
        if (!(await this.canManage(user, id)))
            throw new ForbiddenException("无项目删除权限");
        await this.prisma.project.update({ where: { id }, data: { status: "DELETED" } });
        await this.events.record({ type: "project.deleted", actor: user, projectId: id, message: "删除项目", color: "rose" });
        return { ok: true };
    }
    // ---- Members ----
    async membersOf(user, projectId) {
    ...
    async invite(user, projectId, body) {
        await this.requireProject(user, projectId);
        if (!(await this.canManage(user, projectId)))
            throw new ForbiddenException("无邀请权限");
    ...
    async updateMember(user, projectId, memberId, body) {
        if (!(await this.canManage(user, projectId)))
            throw new ForbiddenException("无成员管理权限");
