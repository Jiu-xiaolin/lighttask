...
let TaskService = class TaskService {
    ...
    async createTask(user, projectId, body) {
        if (!(await this.canEdit(user, projectId)))
            throw new ForbiddenException("无任务创建权限");
    ...
    async updateTask(user, taskId, body) {
        ...
        if (!task)
            throw new NotFoundException("任务不存在");
        if (!(await this.canEdit(user, task.projectId)))
            throw new ForbiddenException("无任务编辑权限");
        ...
        for (const f of dateFields) {
            if (typeof data[f] === "string")
                data[f] = new Date(`${data[f]}T00:00:00Z`);
        }
    ...
    async deleteTask(user, taskId) {
        ...
        if (!task)
            throw new NotFoundException("任务不存在");
        if (!(await this.canEdit(user, task.projectId)))
            throw new ForbiddenException("无任务删除权限");
        await this.prisma.task.update({ where: { id: taskId }, data: { status: "DELETED" } });
    ...
    async copyTask(user, taskId) {
        const task = await this.prisma.task.findUnique({ where: { id: taskId } });
        if (!task)
            throw new NotFoundException("任务不存在");
        if (!(await this.canEdit(user, task.projectId)))
            throw new ForbiddenException("无任务复制权限");
        const lastTask = await this.prisma.task.findFirst({
    ...
    async archiveTask(user, taskId) {
        const task = await this.prisma.task.findUnique({ where: { id: taskId } });
        if (!task || !(await this.canEdit(user, task.projectId)))
            throw new ForbiddenException("无权限");
        await this.prisma.task.update({ where: { id: taskId }, data: { status: "ARCHIVED" } });
    ...
    async restoreTask(user, taskId) {
        const task = await this.prisma.task.findUnique({ where: { id: taskId } });
        if (!task || !(await this.canEdit(user, task.projectId)))
            throw new ForbiddenException("无权限");
        await this.prisma.task.update({ where: { id: taskId }, data: { status: "TODO" } });
    ...
    // ---- Progress ----
    async updateProgress(user, progressId, body, action = "updated") {
        const item = await this.prisma.taskProgress.findUnique({ where: { id: progressId } });
        if (!item)
            throw new NotFoundException("成员进度不存在");
        const member = user.role === "SUPER_ADMIN" ? { role: "admin" } : await this.checkAccess(user, item.projectId);
        if (!member)
            throw new NotFoundException("项目不存在或无权限");
        const canManage = member.role === "owner" || member.role === "admin" || user.role === "SUPER_ADMIN";
        if (!(item.userId === user.id || canManage))
            throw new ForbiddenException("无成员进度更新权限");
        const updateData = { ...body };
        for (const field of ["planStart", "planEnd", "currentEnd", "actualStart", "actualEnd"]) {
            if (typeof updateData[field] === "string")
                updateData[field] = new Date(`${updateData[field]}T00:00:00Z`);
        }
        if (action === "complete") {
            ...
            if (body.currentEnd)
                updateData.currentEnd = new Date(`${body.currentEnd}T00:00:00Z`);
    ...
    async submit(user, progressId, body) {
        const item = await this.prisma.taskProgress.findUnique({ where: { id: progressId } });
        if (!item || item.userId !== user.id)
            throw new ForbiddenException("只能提交自己的任务成果");
